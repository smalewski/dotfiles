XRANDR=$(which xrandr)
AUTORANDR=$(which autorandr)

MONITORS=( $( ${XRANDR} | awk '( $2 == "connected" ){ print $1 }' ) )
AUTORANDR_CONFIGS=( $( ${AUTORANDR} --list ) )
...
for entry in $(seq 0 $((${#AUTORANDR_CONFIGS[@]}-1)))
do
    TILES[$index]="Mode ${AUTORANDR_CONFIGS[$entry]}"
    COMMANDS[$index]="${AUTORANDR} ${AUTORANDR_CONFIGS[$entry]}"
    index+=1
done
